...
class SmetIO : public SnowpackIOInterface {
	...
	private:
		...
		bool enable_vapour_restart_reading; // Jafari added Rho_cum_snow in case of water vapor transport and ENABLE_VAPOUR_RESTART
		bool enable_vapour_restart_writing; // Jafari added Rho_cum_snow in case of water vapor transport and ENABLE_VAPOUR_RESTART
		std::string vapour_transport_model; // Jafari added to make enable_vapour_restart_writing and enable_vapour_restart_reading false if the vapor model is not the coupling with OpenFOAM

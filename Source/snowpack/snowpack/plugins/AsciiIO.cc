...
//TODO: optimize this method. For high-res outputs, we spend more than 50% of the time in this method...
	...
	{				
		...
		// 0902: the water vapor diffusion flux (1.0e-7 kg m-2 s-1)
		fout << "\n0902," << nE + Noffset;
		if (Noffset == 1) fout << "," << std::fixed << std::setprecision(2) << mio::IOUtils::nodata;
		for (size_t e = 0; e < nE; e++)
		...
		// 0906: the averaged temperature over x in OpenFOAM, (degC)
		fout << "\n0906," << nE + Noffset;
		if (Noffset == 1) fout << "," << std::fixed << std::setprecision(2) << mio::IOUtils::nodata;
		for (size_t e = 0; e < nE; e++)
			fout << "," << std::fixed << std::setprecision(2) << IOUtils::K_TO_C(EMS[e].OF_to_SN_Tm);
		
		// 0907: the averaged temperature gradient over x in OpenFOAM, (K m-1)
		fout << "\n0907," << nE + Noffset;
		if (Noffset == 1) fout << "," << std::fixed << std::setprecision(2) << mio::IOUtils::nodata;
		for (size_t e = 0; e < nE; e++)
			fout << "," << std::scientific << std::setprecision(3) << EMS[e].OF_to_SN_gradTm;
		
		// 0908: the averaged Rayleigh number over x in OpenFOAM,  (-)
		fout << "\n0908," << nE + Noffset;
		if (Noffset == 1) fout << "," << std::fixed << std::setprecision(2) << mio::IOUtils::nodata;
		for (size_t e = 0; e < nE; e++)
			fout << "," << std::scientific << std::setprecision(3) << EMS[e].OF_to_SN_Ra;
		
		// 0909: the averaged gas flow velocity over x in OpenFOAM,  (mm s-1)
		fout << "\n0909," << nE + Noffset;
		if (Noffset == 1) fout << "," << std::fixed << std::setprecision(2) << mio::IOUtils::nodata;
		for (size_t e = 0; e < nE; e++)
			fout << "," << std::scientific << std::setprecision(6) << 1.e3*EMS[e].OF_to_SN_Ug;
	}
...
void AsciiIO::writeProHeader(const SnowStation& Xdata, std::ofstream &fout) const
{
	...
	if(1)
	{
		...
		fout << "\n0902,nElems, the water vapor diffusion flux (1.0e-7 kg m-2 s-1)";		
		...
		fout << "\n0906,nElems, the averaged temperature over x in OpenFOAM, (degC)";
		fout << "\n0907,nElems, the averaged temperature gradient over x in OpenFOAM, (K m-1)";
		fout << "\n0908,nElems, the averaged Rayleigh number over x in OpenFOAM,  (-)";
		fout << "\n0909,nElems, the averaged gas flow velocity over x in OpenFOAM,  (mm s-1)";		
	}

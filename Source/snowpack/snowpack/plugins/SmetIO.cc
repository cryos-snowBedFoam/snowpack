...
SmetIO::SmetIO(const SnowpackConfig& cfg, const RunInfo& run_info)
        : fixedPositions(), outpath(), o_snowpath(), experiment(), inpath(), i_snowpath(),
          ...
          out_load(false), out_stab(false), out_canopy(false), out_soileb(false), out_inflate(false), useRichardsEq(false), enable_pref_flow(false), enable_ice_reservoir(false), read_dsm(false), enable_vapour_restart_reading(false), enable_vapour_restart_writing(false)
{
